...
# assume 2 us timing resolution, starting at flash time
tBins = np.linspace(0, 500, 501)
...
# hit threshold is a 5 sigma (assuming noise) count level
hitThreshold = noiseLevel + 5*np.sqrt(noiseLevel)
...
class targetLocHypothesis:
    ...
    def reconstruct_flash(self, xLocs, yLocs, zLocs, tLocs):
        ...
        counts, xBinEdges, yBinEdges = np.histogram2d(
            xLocs, yLocs, bins=[xBins, yBins])
        ...
        self.xSpread = np.sqrt(
            np.sum(np.power(X, 2)*readout.T)/nSel - np.power(self.xCenter, 2))
        self.ySpread = np.sqrt(
            np.sum(np.power(Y, 2)*readout.T)/nSel - np.power(self.yCenter, 2))
        ...
        tReadout = np.where(tReadout > hitThreshold,
                            tReadout - noiseLevel*np.ones_like(tReadout), 0)

        self.tCenter = np.sum(tBinCenters*tReadout)/nSel
        self.tSpread = np.sqrt(
            np.sum(np.power(tBinCenters, 2)*tReadout)/nSel - np.power(self.tCenter, 2))
